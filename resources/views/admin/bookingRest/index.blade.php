...
@can('booking_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.bookings.create') }}?type=rest">
                ...
                <tbody>
                    @foreach($bookings as $key => $booking)
                        <tr data-entry-id="{{ $booking->id }}">
                            ...
                            <td>
                                <span class="{{$booking->status==1?'text-success':($booking->status==2?'text-danger':'')}}">{{$booking->status==1?'Approved':($booking->status==2?'Decline':'Pending')}}</span>
                            </td>
                            <td>
                                @can('booking_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.bookings.show', $booking->id) }}?type=rest">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('booking_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.bookings.edit', $booking->id) }}?type=rest">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan
