...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.kitchentypes.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.kitchentypes.update", [$kitchentypes->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            {{--  @dd($kitchentypes)  --}}
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name_en') ? 'is-invalid' : '' }}" 
                       ...
                       value="{{ old('name_en', $kitchentypes->name_en) }}" 
                        required>
            ...
            <div class="form-group">
                <label class="required" for="name_ar">{{ trans('cruds.kitchentypes.fields.titleAR') }}</label>
                <input class="form-control {{ $errors->has('name_ar') ? 'is-invalid' : '' }}" 
                type="text" name="name_ar" id="name_ar" 
                value="{{ old('name_ar', $kitchentypes->name_ar) }}" 
                 required>
