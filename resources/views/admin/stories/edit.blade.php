    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.stories.update", [$story->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ $story->title }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('restaurant') ? 'is-invalid' : '' }}" name="restaurant_id" id="restaurant_id">
                    @foreach($restaurants as $id => $entry)
                        <option value="{{ $id }}" {{ (old('restaurant_id') ? old('restaurant_id') : $story->restaurant->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label for="ad_duration">{{ trans('cruds.story.fields.ad_duration') }}</label>
                <input class="form-control {{ $errors->has('ad_duration') ? 'is-invalid' : '' }}" type="number" name="ad_duration" id="ad_duration" value="{{ old('ad_duration', $story->ad_duration) }}" required>
                @if($errors->has('ad_duration'))
                    <span class="text-danger">{{ $errors->first('ad_duration') }}</span>
                @endif
            </div>
            <div class="form-group">
