        ...
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('city') ? 'is-invalid' : '' }}" name="city_id" id="city_id">
                    @foreach($cities as $id => $entry)
                        <option value="{{ $id }}" {{ (old('city_id') ? old('city_id') : $user->city_id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('city'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('gender') ? 'is-invalid' : '' }}" name="gender" id="gender">
                    ...
                    <option value="0" {{ (old('gender') ? old('gender') : $user->gender ?? '') == 0 ? 'selected' : '' }}>male</option>
                    <option value="1" {{ (old('gender') ? old('gender') : $user->gender ?? '') == 1 ? 'selected' : '' }}>female</option>
                </select>
